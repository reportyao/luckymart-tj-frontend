-- ============================================
-- 全面修复脚本 V2 - 2026年1月11日
-- 修复以下问题：
-- 1. payment_config 表名不一致
-- 2. lotteries.inventory_product_id 字段类型问题 (处理外键约束)
...
BEGIN
    ...
    FROM pg_constraint
    WHERE conrelid = 'lotteries'::regclass
      AND confrelid = 'inventory_products'::regclass;
    ...
    -- 3. 重新建立外键约束 (使用 TEXT 类型兼容)
    -- 注意：如果 inventory_products.id 是 UUID，我们需要在查询时进行转换，而不是建立硬性外键约束
    -- 或者我们可以建立一个不带强类型检查的约束（PostgreSQL 不支持跨类型外键）
    -- 因此，我们通过应用逻辑来保证一致性，或者在数据库层面使用触发器
    RAISE NOTICE 'Skipping foreign key re-creation to maintain TEXT compatibility';
END $$;
